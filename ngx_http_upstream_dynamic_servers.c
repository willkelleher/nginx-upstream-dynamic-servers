...
static void ngx_http_upstream_dynamic_server_resolve(ngx_event_t *ev) {
    ...
    if (ngx_resolve_name(ctx) != NGX_OK) {
        ...
        ngx_add_timer(&dynamic_server->timer, 5000);
    }
}
...
static void ngx_http_upstream_dynamic_server_resolve_handler(ngx_resolver_ctx_t *ctx) {
    ...
    if (ctx->state) {
        //ngx_log_error(NGX_LOG_ERROR, ctx->resolver->log, 0, "upstream-dynamic-servers: '%V' could not be resolved (%i: %s)", &ctx->name, ctx->state, ngx_resolver_strerror(ctx->state));
